...
```
https://cdn.statically.io/gh/openplatform-labs/images@main/logos/{filename}.svg
```
...
- `logos.json`은 같은 레포에서 동기화 (관리자 → 동기화 버튼)
...
3. **「GitHub에 배포하고 CDN URL 받기」** 클릭
   - `logos/*.svg` + `logos.json` 단일 커밋으로 **openplatform-labs/images** 반영
   - 로컬 SQLite에 메타데이터 저장
...
```env
GITHUB_TOKEN=ghp_xxxxxxxx   # repo 권한 (openplatform-labs/images)
