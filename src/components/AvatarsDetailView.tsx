...
interface AvatarsDetailViewProps {
  name: string;
  shortname: string;
  badgeLabel: string;
  source?: string | null;
...
export function AvatarsDetailView({
  name,
  shortname,
  badgeLabel,
  source,
  ...
  return (
    <div className="avatars-detail pb-16">
      <section className="avatars-detail-stage relative min-h-[min(88svh,900px)] w-full overflow-hidden">
        <button
          type="button"
        ...
        >
          <div className="flex h-full w-full items-center justify-center p-6 md:p-12">
            <div className="avatars-feature-ring relative h-[min(72vw,520px)] w-[min(72vw,520px)] overflow-hidden rounded-full">
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-start justify-between gap-4 px-4 pt-5 md:px-8">
          <Link
            href="/"
            className="pointer-events-auto rounded-full border border-[color:var(--avatars-line)] bg-[color:var(--avatars-studio)]/90 px-3 py-1.5 text-xs font-medium text-[color:var(--avatars-ink)] backdrop-blur-sm transition hover:border-[color:var(--avatars-lime)]"
          >
            ← Roster
          </Link>
          <button
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-[color:var(--avatars-studio)] via-[color:var(--avatars-studio)]/80 to-transparent px-4 pb-8 pt-20 md:px-8">
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[color:var(--avatars-lime)]">
            {badgeLabel}
            {source ? ` · ${source}` : ""}
          </p>
          <h1 className="font-display mt-2 max-w-3xl text-3xl font-semibold tracking-tight text-[color:var(--avatars-ink)] md:text-5xl">
            {name}
          </h1>
      ...
      <section className="mx-auto mt-8 max-w-5xl space-y-5 px-4 md:px-8">
        <div>
          <h2 className="font-display text-lg font-semibold text-[color:var(--avatars-ink)]">
            CDN · 파일
          </h2>
          <p className="mt-1 text-sm text-muted">
            클릭하면 절대경로가 복사됩니다. 프로필은 원형 크롭 CSS와 함께 쓰세요.
          </p>
        </div>
            ...
            return (
              <button
                ...
                className={`rounded-2xl border px-4 py-3 text-left transition ${
                  active
                    ? "border-[color:var(--avatars-lime)] bg-[color:var(--avatars-lime)]/8 ring-1 ring-[color:var(--avatars-lime)]/30"
                    : "border-[color:var(--avatars-line)] bg-[color:var(--avatars-panel)] hover:border-[color:var(--avatars-lime)]/50"
                }`}
              >
        ...
        <div className="rounded-2xl border border-[color:var(--avatars-line)] bg-[color:var(--avatars-panel)] p-5">
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted">
            CDN 절대경로 · {selectedLabel}
            {copiedFilename === selected.filename ? " · 복사됨" : ""}
          </p>
          <code className="block break-all text-sm text-[color:var(--avatars-lime)]">
            {selectedUrl}
          </code>
          <p className="mt-2 text-xs text-muted">
            {[selectedDimensions, selectedBytes].filter(Boolean).join(" · ")}
          </p>
          <pre className="mt-3 overflow-x-auto whitespace-pre-wrap break-all rounded-xl border border-dashed border-[color:var(--avatars-line)] p-3 text-xs text-muted">
{`<img src="${selectedUrl}" alt="${name}" style="width:48px;height:48px;border-radius:9999px;object-fit:cover" />`}
          </pre>
        </div>
      </section>
